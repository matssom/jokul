...
const ScreenExample: VFC = () => {
    ...
    return (
        <>
            <p className="jkl-heading-3 jkl-spacing-2xs--bottom">{`Din dings har en ${getDeviceName()} skjerm i ${
                isLandscape ? "landskaps" : ""
            }${isPortrait ? "portrett" : ""}modus`}</p>

            <p className="jkl-body jkl-spacing-2xs--bottom">{`Oppløsningen på nettleservinduet er ${width} x ${height}`}</p>
        </>
    );
};
